...
def evaluate_model(
    ...
    if max_f1:
        ...
        results["precision"] = sklearn.metrics.precision_score(y, y_hat_best)
        results["recall"] = sklearn.metrics.recall_score(y, y_hat_best)
    return results
...
def compute_per_ts_stats(
    ...
    for unique_id in tqdm.tqdm(df.unique_id.unique()):
        ...
        stats["method"] = df.loc[unique_id].method.values[0]
